    ...
    public class Bank
    {
        ...
        public bool AddAccounts()
        {
			...
			try
			{
				...
				if (answer >= 0)
				{
					bank[bank.Length - 1].SetBalance(answer);
				}
				else
				{
			...
			bank[bank.Length - 1].SetAddress();
            bank[bank.Length - 1].SetSortCode();
            if (bank.Length > 1)
            {
                for (int j = 1; j < bank.Length; j++)
                {
                    if (bank[bank.Length - 1].GetSortCode() == bank[j].GetSortCode() && bank.Length - 1 != j)
                    {
                        bank[bank.Length - 1].SetSortCode();
                        j = 0;
                    }
            ...
            Console.WriteLine("New sort code is: {0}", bank[bank.Length - 1].GetSortCode());
